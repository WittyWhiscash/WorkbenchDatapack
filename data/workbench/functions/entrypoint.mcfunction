...
# If there is an item in every slot needed, and the recipe matches on all three counts, then summon an armor stand with the result (placeholder stone, then modify the data to make it the result item)
    execute if data entity @e[limit=1,tag=workbench_input] HandItems[0].Count if data entity @e[limit=1,tag=workbench_materials] HandItems[0].Count if data entity @e[limit=1,tag=workbench_tool] HandItems[0].Count if score !itemNotFound storage matches 0 if score !materialNotFound storage matches 0 if score !toolNotFound storage matches 0 run summon armor_stand 20 3.6 7 {Invulnerable: true, ShowArms: true, Tags: [workbench, workbench_output], HandItems: [{id: stone, Count: 1b}, {}], NoGravity: true, Invisible: true, Pose:{RightArm:[270f,270f,0f]}}
...
# Otherwise, if a match isn't found, summon the output slot, but with nothing in it.
    execute if data entity @e[limit=1,tag=workbench_input] HandItems[0].Count if data entity @e[limit=1,tag=workbench_materials] HandItems[0].Count if data entity @e[limit=1,tag=workbench_tool] HandItems[0].Count if score !success storage matches 0 run summon armor_stand 20 4 7 {Invulnerable: true, ShowArms: true, Tags: [workbench, workbench_output], HandItems: [{id: air, Count: 1b}, {}], NoGravity: true, Invisible: true,}
