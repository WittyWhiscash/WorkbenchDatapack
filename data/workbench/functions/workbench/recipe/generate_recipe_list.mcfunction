...
# If the recipe marker has not been summoned yet, summon it
    execute if score !recipeMarkerLoaded storage matches 0 run summon marker ~ ~ ~ {CustomName: '{"text":"Recipe Marker"}', CustomNameVisible: true, Tags: [recipe_finder], NoGravity: true}
...
# If the recipe marker has been summoned, run the appropriate recipe generation function based on the slot that's calling this function
    execute if entity @s[tag=workbench_input] run scoreboard players set !lookingForItem storage 1
    ...
    execute if entity @s[tag=workbench_input] run scoreboard players set !lookingForItem storage 0
    execute if entity @s[tag=workbench_materials] run scoreboard players set !lookingForMaterial storage 1
    ...
    execute if entity @s[tag=workbench_materials] run scoreboard players set !lookingForMaterial storage 0
