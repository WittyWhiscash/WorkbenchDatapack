...
# If a match is found, and it isn't comparing against itself, set the iteration result to the current item being iterated on
    ...
    execute if entity @s[tag=workbench_input] if score !success storage matches 1 if score !itemNotFound storage matches 0 run data modify storage work:bench iterationResult.index set from storage work:bench currentIteration.index
    ...
    execute if entity @s[tag=workbench_materials] if score !success storage matches 1 if score !materialNotFound storage matches 0 run data modify storage work:bench iterationResult.index set from storage work:bench currentIteration.index
...
# If there is no success, make the system "think" it found a match so the iteration exits
    ...
    execute if entity @s[tag=workbench_materials] if score !success storage matches 0 run scoreboard players set !materialnotFound storage 0
    execute if entity @s[tag=workbench_tool] if score !success storage matches 0 run scoreboard players set !toolNotFound storage 0
